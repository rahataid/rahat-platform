...
const readAllDbgFiles = () => {
    const dbgFiles = fs.readdirSync(`${__dirname}/../artifacts/src/contracts`);
    const buildFilePaths = [];
    dbgFiles.forEach((file) => {
        const files = fs.readdirSync(`${__dirname}/../artifacts/src/contracts/${file}`);
        files.forEach((f) => {
            if (f.includes('.dbg.json')) {
                //read the content of the file
                //remove .dbg.json from file name string
                const fileName = f.replace('.dbg.json', '');
                const content = JSON.parse(fs.readFileSync(`${__dirname}/../artifacts/src/contracts/${file}/${f}`, 'utf8'));
