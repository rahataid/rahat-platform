...
// true when the number already carries the configured calling code (e.g. an
// "86…" number while COUNTRY_CODE_SETTINGS is "+86"), so we must not add one.
export function hasCallingCode(phone: string, callingCode: string): boolean {
  ...
  // when the number is too malformed to parse, fall back to a prefix match so
  // the configured code is never stacked twice.
...
// i.e. the submitter already stated the country and we must not add one.
